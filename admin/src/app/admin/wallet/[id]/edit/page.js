...
export default function EditWalletPage() {
  ...
  const handleSubmit = async (values) => {
    setLoading(true);
    try {
      await walletService.update(walletId, values);
      message.success(t('wallet_updated', 'کیف پول با موفقیت به‌روزرسانی شد'));
          ...
          <Space>
            ...
            <div>
              <Title level={2} style={{ margin: 0 }}>
                {t('edit_wallet', 'ویرایش کیف پول')}
              </Title>
              <Text type="secondary">
                {wallet?.user?.name || t('edit_wallet_subtitle', 'ویرایش اطلاعات کیف پول')}
              </Text>
            </div>
          </Space>
        </div>
              ...
              >
                ...
                <div style={{ textAlign: 'center' }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    {t('wallet_edit_help', 'تغییرات روی کیف پول اعمال می‌شود')}
                  </Text>
                </div>
              </Card>
            </Col>
