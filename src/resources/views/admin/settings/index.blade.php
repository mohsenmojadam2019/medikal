    ...
    <div class="settings-grid">
        ...
        <div class="settings-card">
            ...
            <form method="POST" action="{{ route('admin.settings.update') }}">
                ...
                <div class="form-group">
                    <label for="app_url">آدرس برنامه</label>
                    <input type="url" class="form-control @error('app_url') is-invalid @enderror"
                           id="app_url" name="app_url" value="{{ old('app_url', $settings['app_url'] ?? config('app.url')) }}" required>
                    @error('app_url')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('timezone') is-invalid @enderror" id="timezone" name="timezone">
                        <option value="Asia/Tehran" {{ (old('timezone', $settings['timezone'] ?? config('app.timezone')) == 'Asia/Tehran') ? 'selected' : '' }}>🇮🇷 تهران</option>
                        <option value="Asia/Dubai" {{ (old('timezone', $settings['timezone'] ?? config('app.timezone')) == 'Asia/Dubai') ? 'selected' : '' }}>🇦🇪 دبی</option>
                        <option value="UTC" {{ (old('timezone', $settings['timezone'] ?? config('app.timezone')) == 'UTC') ? 'selected' : '' }}>🌐 UTC</option>
                        <option value="Europe/London" {{ (old('timezone', $settings['timezone'] ?? config('app.timezone')) == 'Europe/London') ? 'selected' : '' }}>🇬🇧 لندن</option>
                        <option value="America/New_York" {{ (old('timezone', $settings['timezone'] ?? config('app.timezone')) == 'America/New_York') ? 'selected' : '' }}>🇺🇸 نیویورک</option>
                    </select>
                    @error('timezone')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    <label class="form-check">
                        <input type="checkbox" name="app_debug" value="1"
                            {{ old('app_debug', $settings['app_debug'] ?? config('app.debug')) ? 'checked' : '' }}>
                        <span class="form-check-label">
                        <i class="fas fa-bug" style="color:#d97706;"></i>
                        حالت Debug (اشکال‌زدایی) - <small style="color:#94a3b8;">فقط در محیط توسعه فعال باشد</small>
                    </span>
                    </label>
                </div>

                <div class="form-group">
                    <label>محیط فعلی</label>
                    <input type="text" class="form-control" value="{{ $settings['app_env'] ?? config('app.env') }}" disabled style="background:#f1f5f9;">
                    <small style="color:#94a3b8;font-size:0.75rem;">
                        <i class="fas fa-info-circle"></i>
                        محیط: {{ $settings['app_env'] ?? config('app.env') }}
                    </small>
                </div>
