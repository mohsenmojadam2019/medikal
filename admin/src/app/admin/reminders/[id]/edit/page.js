...
export default function EditReminderPage() {
  ...
  useEffect(() => {
    const fetchReminder = async () => {
      try {
        const response = await remindersService.getById(reminderId);
        setReminder(response.data);
        form.setFieldsValue({
          ...response.data,
          date: response.data.date,
          time: response.data.time ? dayjs(response.data.time, 'HH:mm') : null,
        });
      } catch (error) {
  ...
  const handleSubmit = async (values) => {
    setLoading(true);
    try {
      await remindersService.update(reminderId, values);
      message.success(t('reminder_updated', 'یادآوری با موفقیت به‌روزرسانی شد'));
          ...
          <Space>
            ...
            <div>
              <Title level={2} style={{ margin: 0 }}>
                {t('edit_reminder', 'ویرایش یادآوری')}
              </Title>
              <Text type="secondary">
                {t('edit_reminder_subtitle', 'ویرایش اطلاعات یادآوری')}
              </Text>
            </div>
          </Space>
        </div>
              ...
              >
                <Select
                  options={[
                    ...
                    { value: 'sent', label: t('sent', 'ارسال شده') },
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontWeight: 500, marginTop: 4 }}>
                    {reminder?.created_at ? dayjs(reminder.created_at).format('jYYYY/jMM/jDD HH:mm') : '—'}
                  </div>
                </div>
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ fontWeight: 500, marginTop: 4 }}>
                    {reminder?.status || t('pending', 'در انتظار')}
                  </div>
                </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    {t('reminder_edit_help', 'تغییرات روی یادآوری اعمال می‌شود')}
                  </Text>
                </div>
              </Card>
            </Col>
