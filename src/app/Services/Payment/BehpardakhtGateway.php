...
class BehpardakhtGateway extends BaseGateway
{
    ...
    public function initiate(Invoice $invoice, array $options = []): array
    {
        ...
        try {
            ...
            return [
                ...
                'message' => 'در حال انتقال به درگاه به پرداخت...',
            ];

        } catch (\Exception $e) {
            $this->logError('Behpardakht initiation failed', [
                'error' => $e->getMessage(),
                'invoice_id' => $invoice->id,
            ]);
            ...
            return [
                'success' => false,
                'message' => 'خطا در اتصال به درگاه به پرداخت: ' . $e->getMessage(),
                'gateway' => $this->getGatewayName(),
            ];
        }
    }
    ...
    public function verify(Request $request): array
    {
        ...
        if (!$refId) {
            return [
                'success' => false,
                'message' => 'پارامتر RefId یافت نشد',
                'gateway' => $this->getGatewayName(),
            ];
        }
