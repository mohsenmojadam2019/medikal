...
export default function CreateClinicPage() {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (faviconFileList.length > 0) {
        formData.append('favicon', faviconFileList[0].originFileObj);
      }

      await clinicService.create(formData);
      message.success(t('clinic_created', 'کلینیک با موفقیت ایجاد شد'));
      router.push('/admin/clinic');
    } catch (error) {
      console.error('Error creating clinic:', error);
      message.error(t('create_error', 'خطا در ایجاد کلینیک'));
    } finally {
      setLoading(false);
    }
  };
          ...
          <Space>
            ...
            <div>
              <Title level={2} style={{ margin: 0 }}>
                {t('new_clinic', 'کلینیک جدید')}
              </Title>
              <Text type="secondary">
                {t('create_clinic_subtitle', 'ایجاد کلینیک جدید')}
              </Text>
            </div>
          </Space>
        </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  >
                    {logoFileList.length > 0 ? (
                      <img
                        src={URL.createObjectURL(logoFileList[0].originFileObj)}
                        alt="لوگو"
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  >
                    {faviconFileList.length > 0 ? (
                      <img
                        src={URL.createObjectURL(faviconFileList[0].originFileObj)}
                        alt="فاویکون"
                        style={{ width: '100%', height: '100%', objectFit: 'contain' }}
                      />
                    ) : (
