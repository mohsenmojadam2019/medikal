...
export default function InvoicesPage() {
  ...
  const handleReset = () => {
    setSearchText('');
    setFilters({});
    fetchInvoices({ page: 1, search: '', ...filters });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'total_amount',
      render: (total) => total ? `${Number(total).toLocaleString()} تومان` : '—',
      render: (total) => {
          ...
          <Col xs={24} sm={12} md={6}>
            ...
            >
              <Statistic
                ...
                prefix={<FileInvoiceOutlined style={{ color: '#2563eb' }} />}
              />
            </Card>
          </Col>
          <Col xs={24} sm={12} md={6}>
        ...
        {selectedInvoice && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
              <FileInvoiceOutlined style={{ fontSize: 32, color: '#2563eb' }} />
              <div>
            ...
            <Row gutter={[16, 16]}>
              <Col span={12}>
                ...
                <div style={{ fontWeight: 500 }}>{selectedInvoice.patient?.full_name || '—'}</div>
              </Col>
              <Col span={12}>
              ...
              <Col span={12}>
                ...
                <div style={{ fontWeight: 500 }}>
                  {selectedInvoice.created_at ? dayjs(selectedInvoice.created_at).format('jYYYY/jMM/jDD') : '—'}
                </div>
              </Col>
              <Col span={12}>
                <Text type="secondary">{t('paid_at', 'تاریخ پرداخت')}</Text>
                <div style={{ fontWeight: 500 }}>
                  {selectedInvoice.paid_at ? dayjs(selectedInvoice.paid_at).format('jYYYY/jMM/jDD') : '—'}
                </div>
              </Col>
              <Col span={24}>
